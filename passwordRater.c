...
int main(void){
  ...
  while(score!=3){
    score=0;
    printf("A good password will be required to have at least 8 characters, 1 number, one letter, and one punctuation symbol.\n\n");
    printf("Enter your password: ");
    scanf("%s", pass);
    score+=checkLength(pass);
    score+=checkTop(pass);
    score+=checkMix(pass);
  }
...
int checkMix(char *pass){
  int length, i, num, punct, letter;
  num=0;
  punct=0;
  letter=0;
  for(length=0; pass[length]!='\0'; ++length);
  ...
  printf("Your password does not fulfill the requirements of a good password.\n");
  return 0;
}
